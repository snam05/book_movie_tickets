...
import { getMovieById, updateMovie } from '@/lib/api/movies';
...
export default function EditMoviePage() {
  ...
  const loadData = useCallback(async () => {
    try {
      setPageLoading(true);
      const [movie, genresData] = await Promise.all([
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateMovie(Number(movieId), formDataObj);

      router.push('/admin/movies');
    } catch (error) {
      console.error('Error updating movie:', error);
      let errorMessage = 'Lỗi khi cập nhật phim';
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">Cập nhật thông tin phim</p>
      </div>
              ...
              {posterPreview && (
                <div className="space-y-2">
                  <p className="text-sm text-muted-foreground">
                    {posterFile ? 'Poster mới:' : 'Poster hiện tại:'}
                  </p>
                  ...
                  {!posterFile && (
                    <p className="text-xs text-muted-foreground">Upload poster mới để thay thế</p>
                  )}
                </div>
              )}
              <label className="flex items-center justify-center border-2 border-dashed rounded-lg p-6 cursor-pointer hover:border-primary hover:bg-accent transition-colors">
                <div className="text-center">
                  ...
                  <p className="text-sm font-medium">
                    {posterFile ? 'Chọn ảnh khác' : 'Click để chọn hoặc kéo thả ảnh'}
                  </p>
